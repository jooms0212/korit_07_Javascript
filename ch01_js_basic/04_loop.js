...
//   첫 번째, 마지막 index에 해당하는 element
//   다음에도 ','가 찍혀있는데도 불구하고 오류가 발생하지 않는다는 점.

//   두번째, Array의 element인 object의 마지막 property의 value 값
//   다음에도 ','가 있지만 오류가 발생하지 않는다는 점
//   입니다.

//   이거 추후에 property를 추가하거나 혹은 element를 추가하거나 할 때 전에 말한 것처럼 
//   쉼표 치고 엔터 치고 다시 {} 혹은 property 추가를 하기 귀찮으니까 맨 마지막 
//   element/property 여부와 관계없이 쉼표 찍어주는게 매너가 된게 아예 관행처럼 굳어진겁니다.
//   그래서 오류로 처리하지도 않습니다.
...
// // 솔의 눈과 파워에이드 읍료수 합을 구하시오.

// console.log(productList[0].name + '의 가격과 '+ productList[2].name + '의 가격은 각각 ' + productList[0].price + '원과 ' +
//   productList[2].price  + '원으로, 그 합은 ' + (productList[0].price + productList[2].price)  + ' 원 입니다.');

//   // ; 콜론 안해도 오류 안납니다. 하는게 매너

//   let inputCoin = 900;
//   let outputList = [];  // 빈 배열을 하나 선언하겠습니다.
//   // 이제 inputCoin으로 살 수 있는 음료 목록들을 집어넣도록 하겠습니다.
...
//   }
  ...
  /*
    ...
    각 객체의 명확한 상태를 알기 위해서는 결국 배열의 각 element를 추출하는 반복문을 작성해야 한다는 점을
    확인할 수 있습니다.
  */

    // for(let i  =0; i < outputList; i++) {
    //     for( elt j = 0; j < outputList[i; j++]) {
    //       console.log(outputList[i][j]);
    //     }
    // }      이 코드는 논리적으로 말이 되지 않습니다.
    /*
      outputList의 element의 자료형은 객체이고, 객체에는 index number가 존재하지 않으므로 이중 for문을
      통해서 객체 내부의 각 property를 확인하는 것은 불가능합니다.
  ...
  // 그래서 해당 key에 할당된 value를 추출하고 싶다면 121번 라인과 같은 방식으로 코드를 작성해야 합니다.
  ...
  // 3. for-of문
  // for-in문의 경우는 객체 환경으로 생각하시는 편이 낫습니다.
  // 일반적으로 Java에서의 enhanced for문에 해당하는 것은 for-of 문이라고 생각해주시면 좋겠어요

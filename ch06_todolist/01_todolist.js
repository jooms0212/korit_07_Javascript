...
function renderTodos() {
  ...
  todos.forEach((todo, index) => {
    ...
    // 보통은 첫 번째 반복문 돌 때의 element의 이름을 선언합니다.
    ...
    // 그 completed는 자료형이 boolean이었고, 걔가 true / false 인지에 따라서 36번 라인의 조건문의 실행 여부가 결정될겁니다.
    // .css 파일에서 completed 속성 부분을 적용해주기 위해서 쓴 코드라인이 됩니다.
    // completed가 true라면 할 일이 완료되었음을 표시해주기 위해서 css상 차이를 만들었는데, 그걸 적용하기 위한거죠.
      li.classList.add("completed");
    }
    // 36번 라인부터 40번 라인까지의 해석은 JS 객체 상의 key가 completed인 애의 value가 true라면 li 태그에 completed라는 속성을 추가해주는데, 이게 추가 되면 .css에서 정의된 추가 스타일을 적용하기 위한 것입니다(취소선, 배경색 등).
...
function saveTodos() {
  ...
  localStorage.setItem("temp", "안녕"); // 6번 라인 때문에 항상 localStorage의 value가 배열이라고 오해살까봐 추가한 코드 -> 여기는 value가 string이죠 그러면.
}
...
function addTodo() {
  ...
  const newTodo = {
    text: todoText,
    completed: false, // 초기 생성시에무조건 false로 고정
  };
  ...
  // method는 method 하나 당 기능 하나라고 생각하셔야 합니다.
  // 모듈화
}

// '추가' 버튼 클릭 이벤트
addBtn.addEventListener("click", addTodo);  // button 태그에 onclick 속성입니다 -> 두 번재 argument로 addTodo 함수를 넣었는데, addTodo()가 아니라는 점에도 주목해야한다고 했습니다.
